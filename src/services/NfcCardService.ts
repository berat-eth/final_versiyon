...
export class NfcCardService {
  ...
  static async readOnce(timeoutMs: number = 15000): Promise<NfcCardData | null> {
    try {
      console.log('📱 NFC kart okuma başlatılıyor...');
      ...
      try {
        ...
        console.log('✅ NDEF kart okundu:', raw);
        return { raw };
      } catch (ndefError) {
        console.log('⚠️ NDEF okuma başarısız, IsoDep deneniyor...');
        ...
        try {
          ...
          console.log('✅ IsoDep kart okundu:', raw);
          return { raw };
        } catch (isoDepError) {
          console.log('⚠️ IsoDep okuma da başarısız, genel tag okuma deneniyor...');
          ...
          console.log('✅ Genel kart okuma başarılı:', raw);
          return { raw };
        }
      }
    } catch (e) {
      console.error('❌ NFC kart okuma hatası:', e);
      return null;
    } finally {
