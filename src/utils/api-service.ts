...
class ApiService {
  ...
  private async request<T>(
    ...
    try {
      ...
      // Dinamik anahtar/tenantId oku
      try {
        const [storedKey, storedTenant] = await Promise.all([
          getStoredApiKey(),
          getStoredTenantId()
        ]);
        if (storedKey) (headers as any)['X-API-Key'] = storedKey;
        if (storedTenant) (headers as any)['X-Tenant-Id'] = storedTenant;
      } catch {}

      // Eğer güvenli depoda anahtar yoksa, opsiyonel varsayılan tenant anahtarını kullan
      if (!(headers as any)['X-API-Key'] && DEFAULT_TENANT_API_KEY) {
        (headers as any)['X-API-Key'] = DEFAULT_TENANT_API_KEY;
      }
      ...
      if (!response.ok) {
        ...
        return {
          ...
          error: result?.error || String(response.status),
        } as any;
      }
